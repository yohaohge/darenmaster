...
if os.path.exists("ini/th_creator.pickle"):
...
def get_send_msg_time(name, nation, current_user):
    ...
    if name not in user_data.nation_creator_map[nation]:
        ...
        return user_data.nation_creator_map[nation][name]["last_msg_time"]
...
if __name__ == "__main__":
    ...
    for data in nation_map["PH"].values():
        print(data["name"], data["fans"])

    for (user,value) in user_data_manager["GO7"].nation_creator_map["PH"].items():
        print(user, value["last_msg_time"], value["last_invite_time"])